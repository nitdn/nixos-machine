    ...
    options = {
      niri = {
        includes = lib.mkOption {
          ...
          default = [ ];
          description = ''
            Additional non-declarative input paths.
            While this can be used for declarative inputs too (literally
            used in the module definition just look it up), ideally you
            would use the `settings` attrSet or `extraConfig`.
          '';
        };
  ...
  config.perSystem =
    ...
    let
      ...
      finalNiriConfig = lib.strings.concatLines (
        ...
        ++ lib.lists.map (path: ''include "${path}"'') config.niri.includes
      );
    in
    {
      niri.settings = {
      ...
      niri.includes = [
        ''${pkgs.writeText "niri-extraConfig" config.niri.extraConfig}''
      ];
      packages.niri-config = pkgs.writeText "niri-config" finalNiriConfig;
